import re   # ChatGpt told me to import this. This lib helps to find patterns in text and find all words in the book.
...
def load_book_text(pickle_file_path):
    """
    directly load the book text from my pickle file
    """
...
def perform_word_frequency_analysis(book_pickle_path, frequency_output_path, stop_words_file_path):
    ...
    with open(frequency_output_path, 'w', encoding='utf-8') as output_file:
        for word, frequency in word_frequencies.most_common():
            output_file.write(f"{word}: {frequency}\n")
            
            
if __name__ == "__main__":
    perform_word_frequency_analysis('book_text.pkl', 'refined_word_frequencies.txt', 'stop_words.txt')
    print("Word frequency analysis completed and results are saved to refined_word_frequencies.txt.")
