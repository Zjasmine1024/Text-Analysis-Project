...
def load_text(pkl_path):
    ...
    with open(
        pkl_path, "rb"
    ) as f:  # I debugged with ChatGpt, it told me to add 'rb' b/c it's used for reading when dealing with pickled datas
...
def count_mentions_simple(text, chars):
    ...
    for char in chars:
        ...
        matches = re.findall(
            r"\b" + re.escape(lowercases_character) + r"\b", text.lower()
        )
        count = len(matches)
